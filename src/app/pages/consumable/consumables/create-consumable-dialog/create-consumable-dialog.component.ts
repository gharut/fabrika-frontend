...
@Component({
  selector: 'app-create-consumable-dialog',
  styleUrls: ['./create-consumable-dialog.component.scss'],
  templateUrl: './create-consumable-dialog.component.html',
  providers: [CheckboxControlValueAccessor]
})


export class CreateConsumableDialogComponent implements OnInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<CreateConsumableDialogComponent>,
  ...
  ngOnInit() {
    ...
    this.consumableForm = this.formBuilder.group({
      ...
      price_threshold_type: ['', this.validatePriceThresholdType],
      price_threshold: ['', Validators.pattern('^[0-9]*$')],
      qty_threshold: ['', Validators.pattern('^[0-9]*$')],
    });
  ...
  onSubmit() {
    ...
    console.log(this.consumableForm.invalid);
    ...
    this.consumableService.createConsumable(JSON.stringify(this.consumableForm.value)).subscribe({

      next: result => {
        this.formSubmitting = false
        if(result.success) {
          this.toastrService.success("Расходник успешно добавлен!","Изменения сохранены!")
          this.dialogRef.close(result.data);
        }else{
          this.toastrService.error("Расходник не добавлен!", "Произошла ошибка")
